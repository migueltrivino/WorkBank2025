...
async function saveContact(req, res) {
  try {
    ...
    `INSERT INTO contact_messages
        (user_id, nombre, correo, telefono, categoria, asunto, mensaje, adjunto_url, ip_origen)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [user_id || null, nombre, correo, telefono, categoria, asunto, mensaje, adjunto ? `/uploads/contact_files/${adjunto}` : null, ip]
    );
