...
async function saveContact(req, res) {
  try {
    ...
    const allowedCategories = ["contacto", "queja", "sugerencia"];
    if (!allowedCategories.includes(categoria)) {
      return res.status(400).json({
        success: false,
        error: "Categoría no válida. Debe ser 'contacto', 'queja' o 'sugerencia'."
      });
    }
    ...
    await db.execute(
      ...
      [
        ...
        asunto || null,
        mensaje,
        adjunto ? `/uploads/contact_files/${adjunto}` : null
      ]
    );
